...
let CtoF = document.getElementById("CtoF"); CtoF.addEventListener('click', () => { type.innerHTML = 'Celcius to Fahrenheit'; f = document.getElementById("input").value; const output = f.split(","); for (let i = output.length - 1; i >= 0; i--) { output[i] = ((output[i] - 32) / 1.8).toFixed(3); calculation.innerHTML = output } });
